...
the same surface. Only `up`, `image` (build/status/rm), and `config-sync` must
run inside a project (a `.paraspace/` dir, found from `$PWD` up); everything
else — including `para init`, which is how you *create* that dir — works from
anywhere.
...
| `para reconcile [--prune]` | rebuild the registry from Incus (workspaces are [self-describing](./internals.md#workspaces-are-self-describing)); `--prune` drops rows with no live container |
...
| `para image status` | when `$PARA_IMAGE` was last built, and whether its source (payload + `$PARA_IMAGE_BOOTSTRAP` + `$PARA_BASE_IMAGE` + the pre-pulled stack images) has drifted since — i.e. whether a rebuild would produce a different image. Flags an image built incrementally (`-i`) |
| `para image rm` | delete `$PARA_IMAGE` — reclaim space or force a fully clean next build; running workspaces (clones) are unaffected |

`para image-build` remains as a deprecated alias for `para image build`.
...
| `para config-set KEY VALUE` | persist a `PARA_*` override to the user config (`${XDG_CONFIG_HOME:-~/.config}/para/config`). Per-project keys (`PARA_PROJECT`, `PARA_IMAGE`, `PARA_BASE_IMAGE`, …) are refused — they belong in the [Parafile](./parafile.md#1-per-project-keys-are-refused-from-the-user-config) |
