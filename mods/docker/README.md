...
When the mod is added, its host-side `configure` runs
...
malformed, installation still succeeds with a warning and no Parafile edit.
...
empty declaration such as `PARA_ROUTES=""`; edit the Parafile before or after
adding the mod for a manual override. Re-adding the mod reruns inspection but
