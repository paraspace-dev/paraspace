...
Almost every project already encodes its own provisioning somewhere, in CI, a
container build, an infra template, a setup script, or a README section someone
wrote for the last new hire. Adoption is mostly **translation**. Find the
source, translate it, and confirm only the gaps with a human.
...
| `mise.toml`, `.tool-versions`, `.nvmrc`, `.python-version` | exact runtime versions, which you honor rather than rounding to the distro's | `image-build` |
...
**Split by lifetime, not by source.** One question divides the same
information. Does it change per workspace? Package installs, runtime toolchains
and pre-pulled images are the same in every workspace, so they belong in
`image-build` and get paid for once. Clones, credentials, `.env` rendering and
database seeding differ per workspace, so they belong in `provision` and get
paid for on every `up`. Starting things is `boot`.

**A Dockerfile is a package list, not a plan.** Take its `apt-get install` line
and its base-image choice, and ignore its layering, its `COPY`, its `USER` and
its `CMD`. A para workspace is a system container that boots an init and holds
the whole stack, so it sits much closer to a small VM than to an app container.
...
steps name the runtime versions, and the test command is a bring-up you can
copy. What CI won't tell you is anything about the interactive dev loop, so hot
reload, seed data and an admin login are still open questions.
...
and its modules (code-server, dotfiles, an agent CLI) are `image-build` packages
or a para mod. The one thing that does not carry over is the cloud plumbing.
...
the app itself runs on the host, which is the hybrid case in
...
2. **What does a fresh checkout need before it's usable?** Migrations, seed
   data, a test login.
...
5. **Anything heavy that shouldn't be downloaded per workspace?** Model weights,
   a big npm/composer cache, container images.
