...
and it's why an agent can be let off the leash inside one. The code therefore
arrives one of two ways. Either the workspace **pulls** it with a clone over the
network, or the host **pushes** it with a project command from outside.
Everything below is a variation on those two.
...
- **The clone can race guest DNS**, which is what `PARA_READY_HOST` is for. Set
  it to your git host.
- **A repo the Parafile lives in can derive its own origin.** That's one less
  thing to keep in sync, and it stops the scaffolded template's demo URL from
  silently surviving:
  ```sh
  : "${PARA_ORIGIN:=$(git -C "$PARA_PROJECT_DIR" remote get-url origin)}"
  ```
- **Large repos** do better with `git clone --filter=blob:none`, which gives
  full history for a fraction of the transfer and stays a normal repo, unlike
  `--depth 1`, which breaks `git log main..` and some tooling. Clone time is
...
para key <ws>             # a verb void-docker-gh ships: prints the public key
                          # hand it to the human with the URL to paste it at
para up <ws>              # idempotent, so this costs nothing once they confirm
```

For other hosts:

- **GitLab, Bitbucket and Gitea** take the same flow without a CLI. Print the
  key, pause, and the human pastes it into the host's SSH-keys page. The key
  lives on the shared volume, so this happens once per project per machine.
...
- **HTTPS with a token** is for when ssh is blocked. Put the token in the host
  `.env` (`PARA_HOST_ENV` pushes it to `~/.paraspace/host.env`), and have
  `provision` configure a credential helper from it. Never bake a token into the
...
one. Scope it to what you're comfortable with.
...
- **The guest script stays fully single-quoted.** `$PARA_CLONE_DIR` is already
  in the workspace's environment, so nothing has to cross from the host. The
  moment you start escaping `\$HOME` inside double quotes you are one typo from
  a command that runs on the wrong side.
- `para sh -c` is byte-clean when its stdin isn't a terminal, which is what
  makes piping a bundle through it safe.
- The clone's `origin` points at the bundle file, so remove it or repoint it.
...
Both hooks therefore have to tolerate an empty box on that first pass. Guard on
the *tree* rather than on `.git`, because in this case there isn't one:
...
in either direction, and re-running it overwrites files that changed inside the
workspace. If a project lives on this path, the honest advice is that it will
work better once the code is in *some* VCS, even a local-only one, at which
point §2 applies and history comes with it.
...
`svn checkout` and `fossil open` are all fine inside a workspace, and they get
the same idempotence guard.

## 4. Mounting the host directory in
...
If someone wants edits on the host to appear inside, have them run the editor
inside the workspace (`para sh`), or push with the command in §3 when they want
a fresh copy. Reserve the mount for genuinely shared read-only assets, and say
what it costs when you use it.

## 5. Monorepos

Two shapes, both already in para's own `cookbook.md`, so read that rather than
inventing a third. One `.paraspace/` per app gives each app its own project
identity, image and workspaces. One `.paraspace/` at the root uses a `PARA_*`
variable to select which app a workspace boots. The first is right when the apps
have different stacks, the second when they share one.
