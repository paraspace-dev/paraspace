...
a_gh_hook() { # a_gh_hook <hook> <repo> <home> <shared> <fake> <calls>
  env HOME="$3" PATH="$5:$PATH" PARA_HELPERS="$2/libexec/helpers" \
    PARA_HOOKS="$2/mods/gh/hooks" PARA_SHARED="$4" PARA_GH_AUTH=1 \
    PARA_ORIGIN=git@github.com:acme/app.git GH_CALLS="$6" \
    bash "$2/mods/gh/hooks/$1" >/dev/null 2>&1
}
...
test_gh_mod_never_adds_a_key_github_already_holds() {
...
test_gh_mod_shares_its_config_without_the_git_mod() {
  # The link is the mod's own provision hook, so a project that clones itself
