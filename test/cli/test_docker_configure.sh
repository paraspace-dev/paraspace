#!/usr/bin/env bash
# Host-side Docker mod configuration against deterministic Compose JSON.
...
docker_add_model() { # docker_add_model <project> <model> <fake>
  env PARA_PROJECT_DIR="$1" COMPOSE_MODEL="$2" PATH="$3:$PATH" "$PARA" mod add docker 2>&1
}
...
test_docker_configure_infers_one_apex_route_and_explicit_images() {
  ...
  assert_contains "$(cat "$p/.paraspace/Parafile")" 'PARA_PREPULL_IMAGES=example/web:latest' "explicit image inferred" || return 1
  assert_contains "$(cat "$p/.paraspace/Parafile")" 'PARA_ROUTES=8080' "one port uses the apex" || return 1
  assert_not_contains "$(cat "$p/.paraspace/Parafile")" 'worker' "build-only service has no generated image" || return 1
...
test_docker_configure_resolves_multiple_routes_and_skips_unsafe_ports() {
  ...
  parafile="$(cat "$p/.paraspace/Parafile")"
  assert_contains "$parafile" 'PARA_PREPULL_IMAGES=registry.example/app:ready' "resolved duplicate images are removed" || return 1
  assert_eq 1 "$(grep -c '^PARA_PREPULL_IMAGES=' "$p/.paraspace/Parafile")" "one image declaration" || return 1
...
test_docker_configure_keeps_existing_values_independently_and_is_idempotent() {
  ...
  before="$(cat "$p/.paraspace/Parafile")"
  docker_add_model "$p" "$repo/test/fixtures/compose/single.json" "$fake" >/dev/null || return 1
  assert_eq "$before" "$(cat "$p/.paraspace/Parafile")" "re-adding creates no duplicate declarations" || return 1
...
test_docker_configure_skips_all_routes_on_duplicate_normalized_hosts() {
  ...
  assert_not_contains "$(cat "$p/.paraspace/Parafile")" 'PARA_ROUTES=' "ambiguous routes are not guessed" || return 1
  assert_contains "$(cat "$p/.paraspace/Parafile")" 'PARA_PREPULL_IMAGES=one:1\ two:2' "image inference is independent" || return 1
...
test_docker_configure_missing_tools_and_bad_json_are_nonfatal() {
  ...
  out="$(env -i PATH="$empty" PARA_PROJECT_DIR="$p" PARA_MOD_DIR="$repo/mods/docker" PARA_HELPERS="$repo/libexec/helpers" /bin/bash "$repo/mods/docker/configure" 2>&1)" || return 1
  ...
  out="$(PATH="$nocompose:$(dirname "$(command -v node)"):/usr/bin:/bin" PARA_PROJECT_DIR="$p" \
    PARA_MOD_DIR="$repo/mods/docker" PARA_HELPERS="$repo/libexec/helpers" \
    bash "$repo/mods/docker/configure" 2>&1)" || return 1
  assert_contains "$out" "Docker Compose is not available" "missing Compose warns" || return 1

  fake="$(scratch)"; fake_compose "$fake"
  printf 'not json\n' > "$fake/bad.json"
  out="$(COMPOSE_MODEL="$fake/bad.json" PATH="$fake:$PATH" PARA_PROJECT_DIR="$p" PARA_MOD_DIR="$repo/mods/docker" PARA_HELPERS="$repo/libexec/helpers" bash "$repo/mods/docker/configure" 2>&1)" || return 1
  assert_contains "$out" "could not be inspected" "malformed Compose output warns" || return 1
  assert_not_contains "$(cat "$p/.paraspace/Parafile")" 'PARA_PREPULL_IMAGES=' "no edit on malformed output"
}
